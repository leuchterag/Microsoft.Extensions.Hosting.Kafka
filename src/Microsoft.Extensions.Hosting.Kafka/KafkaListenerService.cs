            ...
            await Task.Run(async () =>
            {
                while (!stoppingToken.IsCancellationRequested)
                {
                    ...
                    if (tasks.Count >= maxDegreeOfParallelism)
                    {
                        ...
                        logger.LogDebug($"Waiting for a message handler to finish to fetch the next message since the max degree of paralelism of {maxDegreeOfParallelism} is reached");
                    ...
                    try
                    {
                        ...
                        {
                            logger.LogDebug($"End of topic {msg.Topic} partition {msg.Partition} reached at offset {msg.Offset}");
                        }
                    }
